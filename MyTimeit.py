...
class TimeIt():
    ...
    def _print_log_recursive(self, log_objs, depth=0):
        log_objs = sorted(log_objs, key=lambda x: x.timings, reverse=True)
        for log_obj in log_objs:
            print("%12.6f %12.6f %8i %s %s" % (
                log_obj.timings,
    ...
    def print_summary(self):
        for tname, logs in self._thread_state.items():
            ...
            print("   %-12s %-12s %-8s" % ("ALL", "PER CALL", "COUNT"))
            self._print_log_recursive(logs[0].children.values())
